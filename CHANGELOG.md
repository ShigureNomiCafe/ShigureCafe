...
### Added
- **Backend**: Implemented batch log uploading support for external services.
- **Backend**: Added automated system log retention policy (default 7 days).
- **Bot**: Implemented asynchronous log reporting to the central backend.
- **Configuration**: Added `LOG_RETENTION_DAYS` to global environment variables.

### Changed
- **Backend**: Enhanced database logging with stack trace support and circular logging prevention.
- **Submodules**: Updated `ShigureCafeBackend` to support new logging features.
- **Submodules**: Updated `ShigureCafeBot` to v1.1.1.
...
### Fixed
- **Frontend**: Removed unused imports in `NoticeEditor.vue` that were causing production build failures.

### Changed
- **Submodules**: Updated `ShigureCafeFronted` tracking to the latest commit on the `main` branch.
